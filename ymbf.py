...
ct = datetime.now()
n = ct.month
bulan = ['January', 'February', 'March', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
try:
    if n < 0 or n > 12:
        exit()
    nTemp = n - 1
...
def moch_yayan():
    ...
    print '___________________________________________________________\n';time.sleep(0.03)
    print ' [%s1%s]. Dump ids own'%(O,N);time.sleep(0.03)
    print ' [%s2%s]. Dump ids public'%(O,N);time.sleep(0.03)
    print ' [%s3%s]. Dump ids followers'%(O,N);time.sleep(0.03)
    print ' [%s4%s]. Dump ids like posting'%(O,N);time.sleep(0.03)
    print ' [%s5%s]. Start Hacking'%(O,N);time.sleep(0.03)
    print ' [%s6%s]. Check info'%(O,N);time.sleep(0.03)
    print ' [%s7%s]. crack'%(O,N);time.sleep(0.03)
    print ' [%s8%s]. Settings user agent'%(O,N);time.sleep(0.03)
    print ' [%s9%s]. Info %sscript%s'%(O,N,O,N);time.sleep(0.03)
    print ' [%s0%s]. logout (%stoken%s)'%(M,N,M,N);time.sleep(0.03)
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        ...
        print '\n %s[%s×%s] menu [%s%s%s] tidak ada, check menu nya bro!'%(N,M,N,M,pepek,N);time.sleep(2);moch_yayan()
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_yayan()
        ___yayanganteng___ = raw_input(' [%s?%s] select any one method auto or manual T? [Y/t]: '%(O,N))
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            print '\n [%s+%s] ok ids -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s+%s] cp ids -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                                    try:
